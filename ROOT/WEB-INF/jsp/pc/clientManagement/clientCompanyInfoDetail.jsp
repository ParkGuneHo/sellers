...
var territoryConfirm = "${territoryConfirm}";
if(territoryConfirm != null && territoryConfirm == 0){
	alert("권한이 없습니다.");
		...
		update : function(){
			...
			$.ajax({
				...
				success : function(data){
					...
					$("#textModalCompanySite").val($("#hiddenCompanySite").val());
			...
			//권한설정 select
			$.ajax({ 
				url : "/clientManagement/selectTerritory.do",
				async : false,
				datatype : 'json',
				method : 'POST',
				data : "hiddenModalPK=" + $("#hiddenCompanyId").val() + "&" + params,
				...
				success : function(data){
					var list = data.selectTerritory;
					for(i = 0; i < list.length; i++){
						$("ul.checktree input[data-lv='3'][id='"+list[i].MEMBER_ID_NUM+"']").trigger("click");
					}
