...
//		return clientManagementDAO.gridSalesStatus(map);
	...
	public int updateClientCompanyInfo(Map<String, Object> map, HttpServletRequest request) throws Exception {
		int cnt = 0;
		...
		//권한설정
		if(!CommonUtils.isEmpty(map.get("territoryGroupArr"))) {
			clientManagementDAO.deleteSalesTerritoryAlignMap(map);
			map.put("territoryMemberId", map.get("global_member_id"));
			
			clientManagementDAO.insertSalesTerritoryAlignMap(map);
			String[] territoryGroupArr = (String[])  ((String) map.get("territoryGroupArr")).split(",");
			
			for(String territoryMemberId : territoryGroupArr) {
				if(!territoryMemberId.equals(map.get("global_member_id"))) {
					map.put("territoryMemberId", territoryMemberId);
					clientManagementDAO.insertSalesTerritoryAlignMap(map);
				}
			}
		} else {
			clientManagementDAO.deleteSalesTerritoryAlignMap(map);
			map.put("territoryMemberId", map.get("global_member_id"));	
			clientManagementDAO.insertSalesTerritoryAlignMap(map); //작성자는 기본으로 insert
		}
